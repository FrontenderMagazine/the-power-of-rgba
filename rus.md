...
Но пока поддержка таких CSS-функций не реализована, мы можем [временно][3] использовать PostCSS для компиляции этих функций в обычные цвета. Или мы можем поэкспериментировать и исследовать силу функции`rgba()`, чтобы менять цвета на лету! Давайте посмотрим, как её можно использовать.
